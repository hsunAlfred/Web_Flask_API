...
class nlp_frame:
    def __init__(self) -> None:
        # load stop word
        with open("./Recommender_System_Deploy/rerank/stopwords.txt", encoding='utf-8') as f:
            stopwords = f.read()
        self.custom_stopwords_list = [i for i in stopwords.split('\n')]
        jieba.set_dictionary('./Recommender_System_Deploy/rerank/dict.txt.big')
    ...
    def loadCorpus(self, corpus: str, HMM: bool, use_paddle: bool):
        ...
        if pathlib.Path(f'./Recommender_System_Deploy/rerank/seg_{corpusTarget}_{HMM}_{use_paddle}.xlsx').exists():
            df = pd.read_excel(
                f'./Recommender_System_Deploy/rerank/seg_{corpusTarget}_{HMM}_{use_paddle}.xlsx', usecols=['X', 'y']).dropna()
        else:
            ...
            df.to_excel(
                f'./Recommender_System_Deploy/rerank/seg_{corpusTarget}_{HMM}_{use_paddle}.xlsx', index=False)
